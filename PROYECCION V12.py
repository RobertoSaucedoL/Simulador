...
# --- FUNCIONES DE CÁLCULO Y LÓGICA DE LA APP ---
...
with st.sidebar:
    ...
    # Función auxiliar para mostrar información debajo de los number_input
    ...
    with tab_gastos:
        ...
        with st.expander("💸 Gastos Operativos Individuales", expanded=False):
            # Obtener todas las subcuentas de gastos operativos para mostrarlas
            gastos_operativos = estructura['TOTAL GASTOS OPERATIVOS']['subcuentas']
            for gasto, datos in gastos_operativos.items():
                key = f"sim_{gasto.replace(' ', '_')}"
                actual_val = datos['actual']
                meta_val = datos['meta']
                min_val = -float(actual_val * 3) if actual_val > 0 else -100000.0
                max_val = float(actual_val * 3) if actual_val > 0 else 100000.0
                st.number_input(
                    label=f"{gasto}",
                    min_value=min_val,
                    max_value=max_val,
                    value=st.session_state.get(key, 0.0),
                    step=max(1.0, actual_val*0.05), # Step dinámico
                    key=key,
                    format="%f" # Para mostrar decimales si los hay
                )
                display_number_input_info_with_actual_meta_brecha(key, actual_val, meta_val)


    with tab_otros:
        st.subheader("Otros Gastos y Financieros")
        # Control para TOTAL DE OTROS GASTOS
        key_otros_gastos = 'sim_TOTAL_DE_OTROS_GASTOS'
        actual_val_og = get_actual_value(estructura, 'TOTAL DE OTROS GASTOS')
        meta_val_og = get_meta_value(estructura, 'TOTAL DE OTROS GASTOS')
        st.number_input(
            "Total de Otros Gastos",
            value=st.session_state.get(key_otros_gastos, 0.0),
            step=1000.0,
            key=key_otros_gastos
        )
        display_number_input_info_with_actual_meta_brecha(key_otros_gastos, actual_val_og, meta_val_og)

        st.markdown("---")
        # Controles para la sección de FINANCIEROS
        for item, datos in estructura['FINANCIEROS']['subcuentas'].items():
            key = f"sim_{item.replace(' ', '_')}"
            actual_val = datos['actual']
            meta_val = datos['meta']
            min_val = -abs(actual_val * 5) if actual_val != 0 else -1000000
            max_val = abs(actual_val * 5) if actual_val != 0 else 1000000
            st.number_input(
                label=f"{item}",
                min_value=min_val,
                max_value=max_val,
                value=st.session_state.get(key, 0.0),
                step=max(1.0, abs(actual_val*0.05)),
                key=key,
                format="%f"
            )
            display_number_input_info_with_actual_meta_brecha(key, actual_val, meta_val)
